...
export default (minutes: number = 0) => {
  ...
  const setVoteCookie = (vote: Vote) => {
    voteCookie.value = vote;
    localStorage.setItem("voteId", JSON.stringify(vote.id));
  };
  const getVoteId = (): null | string => localStorage.getItem("voteId");
  const clearVoteId = () => localStorage.removeItem("voteId");
